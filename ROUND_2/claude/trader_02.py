# trader.py — IMC Prosperity 4, Round 2 (Enhanced: delayed unwind + tighter OSM L1)
# Key changes:
# 1. ENDGAME_START raised to 99_000 (price rises monotonically; old 94_500 sold too early)
# 2. OSM L1 fixed at 3 ticks bid/ask (avg spread=16, fixed 3 > dynamic 3-5 for fill rate)
# 3. Pepper unwind pace increased to 4× (less time, need faster liquidation)
...
class Trader:
    ...
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 15
    # CHANGE 1: Delay endgame start — pepper peaks at ~ts=99,500, not 94,500.
    # Old value caused 5,000 ticks of premature selling (costs ~80 units × 5 tick rise ≈ 400 XIREC).
    ENDGAME_START  = 99_000
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
                ...
                for ask_px in sorted(od.sell_orders.keys()):
                    ...
                    if ask_px <= fair + self.PEPPER_BUY_TOL:
                        ...
                        if vol > 0:
                            ...
                            buy_cap -= vol
                if buy_cap > 0 and od.sell_orders:
                    best_ask_px = min(od.sell_orders.keys())
                    orders.append(Order(self.PEPPER, best_ask_px, buy_cap))
            ...
            # ── Endgame Unwind ─────────────────────────────────────────
            # CHANGE 1 effect: only 900 ticks remain, must unwind 80 units fast.
            # Increased pace multiplier to 4× (was 2.5×) to ensure full exit.
            if pos > 0 and od.buy_orders:
                ticks_left    = max(1, (self.MAX_TS - ts) // 100 + 1)
                per_tick_sell = math.ceil(pos / ticks_left)
                to_sell       = min(pos, int(per_tick_sell * 4.0))
                remaining = to_sell
                ...
                if ts >= self.MAX_TS - 100 and pos > 0:
                    ...
                    if leftover > 0 and od.buy_orders:
                        best_bid_px = max(od.buy_orders.keys())
                        orders.append(Order(self.PEPPER, best_bid_px, -leftover))
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # CHANGE 2: Fixed L1 at 3 ticks instead of dynamic 3-5.
        # Avg spread=16; fixed 3-tick L1 sits inside the spread while maximizing fill rate.
        # Dynamic formula sometimes pushed to 5 ticks (low fill) or matched inside spread badly.
        l1_bid_offset = 3
        l1_ask_offset = 3
