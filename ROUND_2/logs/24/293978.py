# trader.py — IMC Prosperity 4, Round 2 (Experiment A: Pepper Dip Scalp)
# 8,240 baseline + pattern‑driven scalp: buy dips of 8+ ticks, sell 5+ ticks higher.
...
class Trader:
    ...
    # ── PEPPER (8,240 baseline + scalp state) ─────────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 15
    ENDGAME_START  = 94_500

    # ── OSMIUM (8,240 baseline, unchanged) ────────────────────────────
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
            ...
            # Entry: price dips 8+ ticks from recent high, and we have room
            ...
            # Exit: price rises 5+ ticks from entry
            if scalp_entry is not None and best_bid and best_bid > scalp_entry + 5:
                ...
                if vol > 0:
                    ...
                    # Reset recent high to current mid after a successful scalp
                    data["pepper_recent_high"] = mid
