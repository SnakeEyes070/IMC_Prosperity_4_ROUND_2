...
class Trader:
    ...
    # --- Pepper (Claude‑calibrated) ---
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 5                # Reduced from 12 → avoids overpaying
    ENDGAME_START = 90_000            # Earlier: sells into 13,087–13,092 bids

    # --- Osmium (Claude‑calibrated) ---
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA = 0.02
    OSM_PASSIVE_BID_OFFSET = 6        # Fixed 6 ticks below mid (leads book)
    OSM_PASSIVE_ASK_OFFSET = 6        # Fixed 6 ticks above mid
    OSM_PASSIVE_SIZE = 19
    OSM_MR_THRESH = 8
    OSM_MR_MAX_QTY = 24
    OSM_SKEW_FACTOR = 0.06
    OSM_AGGRESSIVE_BUY_THRESH = 10_000   # Raised: captures ask≤10,000
    OSM_AGGRESSIVE_SELL_THRESH = 10_003  # Lowered: captures 10 missed sells
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Aggressive taking (Claude thresholds)
        ...
        # Passive market making (Claude fixed offsets)
        if buy_cap > 0:
