# trader.py — IMC Prosperity 4, Round 2 (Claude Forensic v8,643 Optimized)
import json
import math
...
class Trader:
    ...
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 3                # ← 5 → 3 (reduces overpay from 1.16 ticks/unit)
    ENDGAME_START = 83_000            # ← 90k → 83k (captures premium bids earlier)
    SCALP_RESERVE = 6
    ...
    OSM_EMA_ALPHA = 0.02
    OSM_PASSIVE_BID_OFFSET = 5        # ← 6 → 5 (better queue position)
    OSM_PASSIVE_ASK_OFFSET = 5        # ← 6 → 5
    OSM_PASSIVE_SIZE = 19
    OSM_MR_THRESH = 8
    OSM_MR_MAX_QTY = 24
    OSM_SKEW_FACTOR = 0.06
    OSM_AGGRESSIVE_BUY_THRESH = 9_998   # ← 10,000 → 9,998 (Claude: saves 1–2 ticks)
    OSM_AGGRESSIVE_SELL_THRESH = 10_002  # ← 10,003 → 10,002 (recovers 12 missed sells)

    # Laddered aggression + inventory skew
    OSM_AGG_SIZE_L1 = 12
    OSM_AGG_SIZE_L2 = 8
    OSM_AGG_DEEPER = 2
