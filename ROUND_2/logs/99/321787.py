# trader.py — IMC Prosperity 4, Round 2 (Fully Forensic‑Optimized)
import json
import math
...
class Trader:
    ...
    # --- Pepper (forensic calibrated) ---
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 3                # Tighter: reduces overpay (was 5)
    ENDGAME_START = 99_000            # Hold until final 900 ticks (was 90k)

    # --- Osmium (forensic calibrated) ---
    OSM_FAIR_FALLBACK = 10_004        # Observed mean (was 10_000)
    OSM_EMA_ALPHA = 0.10              # Faster tracking (was 0.02)
    OSM_PASSIVE_BID_OFFSET = 5        # Tighter (was 6)
    OSM_PASSIVE_ASK_OFFSET = 5        # Tighter (was 6)
    OSM_PASSIVE_SIZE = 19
    OSM_MR_THRESH = 8
    OSM_MR_MAX_QTY = 24
    OSM_SKEW_FACTOR = 0.06            # Now actually used
    OSM_AGGRESSIVE_BUY_THRESH = 9_998
    OSM_AGGRESSIVE_SELL_THRESH = 10_005  # Higher quality sells (was 10_003)
    OSM_SELL_COOLDOWN = 300           # Prevent flooding
    ...
    def bid(self) -> int:
        return 5_100                   # Rook‑E1 break‑even (was 6,500)
    ...
    # ---------- Pepper (scalp removed, core only) ----------
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Aggressive selling with cooldown
        ...
        # Passive market making with INVENTORY SKEW (finally applied)
