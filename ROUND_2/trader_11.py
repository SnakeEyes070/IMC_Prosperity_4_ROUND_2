# trader.py — IMC Prosperity 4, Round 2 (15k+ Stacked Strategy)
# Implements: Multi‑day anchor reset + Micro‑price + Imbalance filter + Asymmetric MM
...
class Trader:
    ...
    # ═══════════════════════════════════════════════════════════════
    # OSMIUM – Micro‑price + Asymmetric MM + Imbalance Filter
    # ═══════════════════════════════════════════════════════════════
    ...
    # Imbalance filter: skip passive quotes when |imbalance| > this
    IMBALANCE_THRESH = 0.30

    # ═══════════════════════════════════════════════════════════════
    # DAY DETECTION (Multi‑Day Anchor Reset)
    # ═══════════════════════════════════════════════════════════════
    ROUND_DAYS     = 3
    MAX_TS         = 9_900
    NEW_DAY_THRESH = 1_000
    PRICE_GAP_THRESH = 80   # Pepper gaps ~100 pts between days

    # ═══════════════════════════════════════════════════════════════
    # MARKET ACCESS FEE – Game Theory Optimal
    # ═══════════════════════════════════════════════════════════════
    def bid(self) -> int:
        return 6_500   # slightly higher to guarantee top 50%
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # Multi‑day detection: timestamp reset OR large price gap
        ...
        if self.PEPPER in state.order_depths:
            ...
            if od.sell_orders and od.buy_orders:
                mid = (min(od.sell_orders.keys()) + max(od.buy_orders.keys())) / 2.0
                prev_mid = data.get("pepper_mid", mid)
                if prev_mid - mid > self.PRICE_GAP_THRESH:
                    new_day_detected = True
                data["pepper_mid"] = mid
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Passive asymmetric multi‑level quotes with imbalance filter
        ...
        for (b_off, a_off, base_sz) in self.OSM_LEVELS:
            ...
            # Imbalance filter: only post if pressure isn't strongly against us
