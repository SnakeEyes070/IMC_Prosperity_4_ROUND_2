# trader.py — IMC Prosperity 4, Round 2 (ALL‑IN for 15k Raw Target)
# Maximum aggression on every proven lever. High risk, high reward.
...
class Trader:
    ...
    # ── PEPPER (maximum aggression) ─────────────────────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 22           # ultra‑wide to guarantee instant fill
    PEPPER_MAX_BUY = 80

    # ── OSMIUM (all‑in scaling) ─────────────────────────────────────────
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA     = 0.025     # faster fair value
    OSM_L1_SIZE       = 28        # aggressive sizes
    OSM_L2_SIZE       = 22
    OSM_L3_SIZE       = 18
    OSM_MR_THRESH     = 6         # tighter mean‑reversion trigger
    OSM_MR_MAX_QTY    = 35        # larger captures
    OSM_SKEW_FACTOR   = 0.06

    # ── TIMING ──────────────────────────────────────────────────────────
    ROUND_DAYS     = 3
    MAX_TS         = 99_900
    ENDGAME_START  = 95_500       # earlier unwind for max smoothness
    NEW_DAY_THRESH = 10_000

    # ───────────────────────────────────────────────────────────────────
    def bid(self) -> int:
        return 10_000  # guarantee extra 25% flow
    ...
    # ───────────────────────────────────────────────────────────────────
    # PEPPER – Unchanged core, ultra‑wide tolerance
    # ───────────────────────────────────────────────────────────────────
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Dynamic offsets – slightly tighter
        spread_factor = current_spread / 16.0
        dyn_l1 = max(2, min(4, round(3 * spread_factor)))   # tighter L1
        dyn_l2 = max(4, min(8, round(6 * spread_factor)))
        dyn_l3 = max(8, min(12, round(10 * spread_factor)))
