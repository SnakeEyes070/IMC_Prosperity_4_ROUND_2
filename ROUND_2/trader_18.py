# trader.py — IMC Prosperity 4, Round 2 (Data‑Derived Parameters)
# All values extracted from statistical analysis of the Round 2 data capsule.
...
class Trader:
    ...
    # ── PEPPER (data‑derived) ──────────────────────────────────────────
    PEPPER_SLOPE   = 0.001011     # from linear regression
    PEPPER_BUY_TOL = 12           # opening spread ~13, so 12 is optimal

    # ── OSMIUM (data‑derived) ──────────────────────────────────────────
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA     = 0.067     # half‑life ~10 ticks
    OSM_L1_SIZE       = 14        # matches typical L1 volume
    OSM_L2_SIZE       = 10
    OSM_L3_SIZE       = 7
    OSM_MR_THRESH     = 8         # 1.6σ, already optimal
    OSM_MR_MAX_QTY    = 24
    ...
    ROUND_DAYS     = 3
    MAX_TS         = 99_900
    ENDGAME_START  = 96_600       # slightly later peak
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Asymmetric L1: buy deeper, sell tighter
