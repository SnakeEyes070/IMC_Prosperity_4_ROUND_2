# trader.py — IMC Prosperity 4, Round 2 (Data‑Derived Final)
# All parameters extracted from statistical analysis of the Round 2 data capsule.
...
class Trader:
    ...
    # ── PEPPER (data‑derived) ──────────────────────────────────────────
    PEPPER_SLOPE   = 0.0001001   # 100 pts over 999,900 ticks
    PEPPER_BUY_TOL = 11          # opening spread ~14, so 11 is optimal

    # ── OSMIUM (data‑derived) ──────────────────────────────────────────
    OSM_FAIR_FALLBACK = 10_001   # from mean analysis
    OSM_EMA_ALPHA     = 0.067    # half‑life ~10 ticks
    OSM_L1_SIZE       = 10       # slightly above analysis for more flow
    OSM_L2_SIZE       = 7
    OSM_L3_SIZE       = 5
    OSM_MR_THRESH     = 7        # 1.5σ based on std dev 4.59
    OSM_MR_MAX_QTY    = 20
    OSM_SKEW_FACTOR   = 0.06

    # ── TIMING (corrected for Round 2 scale) ───────────────────────────
    ROUND_DAYS     = 3
    MAX_TS         = 999_900     # Round 2 has 10× larger timestamps
    ENDGAME_START  = 999_000     # unwind over final ~1,000 ticks
    NEW_DAY_THRESH = 100_000     # day rollover detection
    ...
    # ──────────────────────────────────────────────────────────────────
    # OSMIUM – Asymmetric L1 + data‑derived sizes/alpha
    # ──────────────────────────────────────────────────────────────────
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Asymmetric L1: buy deeper, sell tighter (exploits -0.57 autocorr)
