# trader.py — IMC Prosperity 4, Round 2 (Recalibrated Safe Aggression)
# Restores 8,199 baseline + keeps micro‑price + relaxed imbalance filter
...
class Trader:
    ...
    # ── PEPPER (Restored tolerance, proven unwind) ───────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 16           # slightly wider than 15, but safe
    PEPPER_MAX_BUY = 80

    # ── OSMIUM (Keep micro‑price, restore sizes, relaxed filter) ─────────
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA     = 0.015
    OSM_L1_SIZE       = 18        # restored
    OSM_L2_SIZE       = 14
    OSM_L3_SIZE       = 10
    OSM_MR_THRESH     = 8         # restored
    OSM_MR_MAX_QTY    = 24
    OSM_SKEW_FACTOR   = 0.06
    OSM_IMBALANCE_THRESH = 0.35  # relaxed – only block extreme pressure

    # ── TIMING ───────────────────────────────────────────────────────────
    ROUND_DAYS     = 3
    MAX_TS         = 99_900
    ENDGAME_START  = 96_500      # restored
    NEW_DAY_THRESH = 10_000

    # ──────────────────────────────────────────────────────────────────────
    # bid() – Market Access Fee (balanced game‑theory)
    # 5,000 clears median with margin, preserves capital better than 5,500.
    # ──────────────────────────────────────────────────────────────────────
    def bid(self) -> int:
        return 5000
    ...
    # ──────────────────────────────────────────────────────────────────────
    # OSMIUM – Micro‑price + Dynamic Spread + Relaxed Imbalance Filter
    # ──────────────────────────────────────────────────────────────────────
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        for offset, base_size in levels:
            ...
            # Relaxed imbalance filter – only block extreme pressure
