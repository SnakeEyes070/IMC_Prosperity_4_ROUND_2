# trader.py — IMC Prosperity 4, Round 2 (Log‑Calibrated Final)
# Precision micro‑adjustments based on 8,240 run log analysis.
...
class Trader:
    ...
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 15
    ENDGAME_START  = 96_000       # 500 ticks earlier for smoother exit
    ...
    OSM_L2_SIZE       = 14
    OSM_L3_SIZE       = 10
    OSM_MR_THRESH     = 7          # reduced from 8 to capture more reversions
    OSM_MR_MAX_QTY    = 24
    ...
    # ──────────────────────────────────────────────────────────────────
    def bid(self) -> int:
        return 6_500                # higher MAF bid to secure extra flow
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Aggressive mean reversion (log‑optimized threshold = 7)
        ...
        # Dynamic offsets (log‑optimized: L1 fixed at 3, L2/L3 remain dynamic)
        spread_factor = current_spread / 16.0
        l1_offset = 3                                        # fixed at 3 ticks
        l2_offset = max(5, min(9, round(7 * spread_factor)))
        l3_offset = max(9, min(13, round(11 * spread_factor)))

        levels = [
            (l1_offset, self.OSM_L1_SIZE),
            (l2_offset, self.OSM_L2_SIZE),
            (l3_offset, self.OSM_L3_SIZE),
        ]
