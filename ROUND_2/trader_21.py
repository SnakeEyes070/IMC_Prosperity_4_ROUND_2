# trader.py — IMC Prosperity 4, Round 2 (Unwind‑Calibrated Final)
# Based on log volume profile: earlier unwind + slightly faster pace.
...
class Trader:
    ...
    # ── PEPPER (unwind‑calibrated) ─────────────────────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 15
    ENDGAME_START  = 95_000       # earlier: spread sales over thicker volume
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
            ...
            if pos > 0 and od.buy_orders:
                ...
                to_sell       = min(pos, int(per_tick_sell * 2.5))   # faster pace when volume allows
                remaining = to_sell
