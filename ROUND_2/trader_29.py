# trader.py — Experiment B + C Combined
# 8,364 baseline + Osmium spread sizing + Pepper volume‑triggered unwind.
...
class Trader:
    ...
    # ── PEPPER (8,364 baseline) ───────────────────────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 15
    ENDGAME_START  = 94_500

    # ── OSMIUM (baseline sizes) ───────────────────────────────────────
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
            ...
            # ─── EXPERIMENT C: Volume‑Triggered Pace ─────────────────────
            if pos > 0 and od.buy_orders:
                best_bid_vol = od.buy_orders.get(best_bid, 0) if best_bid else 0
                pace_mult = 3.5 if best_bid_vol < 8 else 2.5
                
                ticks_left    = max(1, (self.MAX_TS - ts) // 100 + 1)
                per_tick_sell = math.ceil(pos / ticks_left)
                to_sell       = min(pos, int(per_tick_sell * pace_mult))
                remaining = to_sell
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # ─── EXPERIMENT B: Spread‑Based Size Multiplier ─────────────────
        spread_mult = 1.5 if current_spread >= 18 else 1.0

        levels = [
            (l1_bid_offset, l1_ask_offset, int(self.OSM_L1_SIZE * spread_mult)),
            (base_l2, base_l2, int(self.OSM_L2_SIZE * spread_mult)),
            (base_l3, base_l3, int(self.OSM_L3_SIZE * spread_mult)),
        ]
