# trader.py — 8,364 Baseline + Osmium Micro‑Scalp (Experiment D)
...
class Trader:
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Aggressive mean reversion (8 ticks)
        ...
        # ─── EXPERIMENT D: Osmium Micro‑Scalp (6 ticks) ─────────────────
        if best_ask and best_ask < fair - 6 and buy_cap > 0:
            vol = min(8, buy_cap, -od.sell_orders.get(best_ask, 0))
            if vol > 0:
                orders.append(Order(self.OSMIUM, best_ask, vol))
                buy_cap -= vol

        if best_bid and best_bid > fair + 6 and sell_cap > 0:
            vol = min(8, sell_cap, od.buy_orders.get(best_bid, 0))
            if vol > 0:
