# trader.py — IMC Prosperity 4, Round 2 (Experiment A3: Slippage‑Aware Entry)
# Sweep only top 2 levels, then post passive bid for remainder.
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if prev_ts > self.NEW_DAY_THRESH and ts < self.NEW_DAY_THRESH:
            ...
            data.pop("pepper_passive_phase", None)
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
            ...
            # ── Phase 1: Sweep only top 2 ask levels ──────────────────
            if buy_cap > 0 and od.sell_orders:
                levels_swept = 0
                for ask_px in sorted(od.sell_orders.keys()):
                    if buy_cap <= 0 or levels_swept >= 2:
                        break
            ...
            # ── Phase 2: Passive bid for remaining capacity ───────────
            if buy_cap > 0 and best_ask:
                # Post a bid just below the best ask to capture incoming sellers
                passive_bid = best_ask - 1
                orders.append(Order(self.PEPPER, passive_bid, buy_cap))

            # ── Scalp (unchanged from 8,364) ──────────────────────────
            ...
            if scalp_entry is None and best_ask and best_ask < data["pepper_recent_high"] - 10:
                remaining_cap = self.LIMIT - pos
                if remaining_cap > 0:
                    vol = min(12, remaining_cap, -od.sell_orders.get(best_ask, 0))
                    if vol > 0:
                        orders.append(Order(self.PEPPER, best_ask, vol))
                        data["pepper_scalp_entry"] = best_ask

            if scalp_entry is not None and best_bid and best_bid > scalp_entry + 6:
                vol = min(12, od.buy_orders.get(best_bid, 0))
                if vol > 0:
