# trader.py — IMC Prosperity 4, Round 2 (Final Calibrated)
# 8,434 baseline + scalp size 11.
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
            ...
            if scalp_entry is None and best_ask and best_ask < data["pepper_recent_high"] - 8:
                ...
                if remaining_cap > 0:
                    vol = min(11, remaining_cap, -od.sell_orders.get(best_ask, 0))
                    if vol > 0:
            ...
            if scalp_entry is not None and best_bid and best_bid > scalp_entry + 5:
                vol = min(11, od.buy_orders.get(best_bid, 0))
                if vol > 0:
