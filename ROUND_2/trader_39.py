# trader.py — IMC Prosperity 4, Round 2 (Multi‑Level Entry Ladder)
# 8,434 baseline + laddered Pepper entry to reduce slippage.
...
class Trader:
    ...
    # ── PEPPER (8,434 baseline + entry ladder) ────────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 12
    ENDGAME_START  = 94_000

    # Multi‑level entry ladder for Pepper
    # (price offset from best ask, quantity)
    PEPPER_ENTRY_LADDER = [
        (0, 30),    # Best ask: 30 units
        (2, 25),    # Ask + 2: 25 units
        (4, 15),    # Ask + 4: 15 units
        (6, 10),    # Ask + 6: 10 units (total 80)
    ]

    # ── OSMIUM (8,434 baseline, unchanged) ────────────────────────────
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
            ...
            # ─── MULTI‑LEVEL ENTRY LADDER ─────────────────────────────
            if buy_cap > 0 and best_ask:
                for offset, qty in self.PEPPER_ENTRY_LADDER:
                    if buy_cap <= 0:
                        break
                    target_price = best_ask + offset
                    # Only buy if price is within fair + tolerance
                    if target_price <= fair + self.PEPPER_BUY_TOL:
                        available = -od.sell_orders.get(target_price, 0)
                        vol = min(qty, buy_cap, available)
                        if vol > 0:
                            orders.append(Order(self.PEPPER, target_price, vol))
                            buy_cap -= vol

                # Fallback: if still not full, post passive bid
                if buy_cap > 0:
