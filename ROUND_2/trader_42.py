# trader.py — IMC Prosperity 4, Round 2 (Log‑Calibrated 8,364 Baseline)
# Micro‑adjustments based on your log analysis.
...
class Trader:
    ...
    # ── PEPPER (log‑calibrated) ───────────────────────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 5          # Reduced from 15 (log shows fills at 13007-13008)
    ENDGAME_START  = 90_000      # Earlier: volume drops to 5 by 97,400

    # ── OSMIUM (log‑calibrated) ───────────────────────────────────────
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA     = 0.02     # Slightly faster (from previous test)
    OSM_L1_SIZE       = 19
    OSM_L2_SIZE       = 14
    OSM_L3_SIZE       = 10
    OSM_MR_THRESH     = 8        # Keep original MR threshold
    OSM_MR_MAX_QTY    = 24
    OSM_SKEW_FACTOR   = 0.06
    
    # New explicit take thresholds (from log extremes)
    OSM_AGGRESSIVE_BUY_THRESH  = 9_998   # Buy if ask <= this
    OSM_AGGRESSIVE_SELL_THRESH = 10_002  # Sell if bid >= this
    ...
    def _pepper_orders(self, state: TradingState, ts: int, fair: float,
        ...
        if not is_endgame:
            ...
            if pos > 0 and od.buy_orders:
                ...
                to_sell       = min(pos, int(per_tick_sell * 1.8))
                remaining = to_sell
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # ─── NEW: Aggressive taking based on log extremes ───────────────
        if best_ask and best_ask <= self.OSM_AGGRESSIVE_BUY_THRESH and buy_cap > 0:
            vol = min(self.OSM_MR_MAX_QTY, buy_cap, -od.sell_orders.get(best_ask, 0))
            if vol > 0:
                orders.append(Order(self.OSMIUM, best_ask, vol))
                buy_cap -= vol

        if best_bid and best_bid >= self.OSM_AGGRESSIVE_SELL_THRESH and sell_cap > 0:
            vol = min(self.OSM_MR_MAX_QTY, sell_cap, od.buy_orders.get(best_bid, 0))
            if vol > 0:
                orders.append(Order(self.OSMIUM, best_bid, -vol))
                sell_cap -= vol

        # ─── Original Mean Reversion (kept as safety net) ───────────────
