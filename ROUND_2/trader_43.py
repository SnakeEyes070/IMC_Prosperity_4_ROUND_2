# trader.py — IMC Prosperity 4, Round 2 (Precision Fix: L1 Offset = 2)
import json
import math
...
class Trader:
    ...
    # Pepper (unchanged from 8435 baseline)
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 12
    ENDGAME_START = 94_000

    # Osmium (unchanged except L1 offset)
    ...
    OSM_FIXED_L1_OFFSET = 2   # ← Only change: fixed 2 ticks instead of dynamic 3–5
    ...
    # ---------- Osmium Methods (Only L1 Offset Changed) ----------
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # L1 uses fixed offset 2
        l1_bid_offset = self.OSM_FIXED_L1_OFFSET + 1  # asymmetric: buy deeper
