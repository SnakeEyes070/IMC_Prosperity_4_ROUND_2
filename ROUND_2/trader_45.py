...
# Combines: asymmetric lite + scalp + forensic calibrations + spread anticipation
import json
import math
...
class Trader:
    ...
    # --- Pepper (forensically calibrated) ---
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 8                # Tighter: reduces overpayment (was 12)
    ENDGAME_START = 92_000            # Earlier: smoother exit (was 94,000)
    UNWIND_PACE = 2.0                 # Slightly slower: 2.0× (was 2.5×)

    # --- Osmium (asymmetric lite + fixed L1) ---
    ...
    OSM_AGGRESSIVE_BUY_THRESH = 9_998     # Slightly wider (was 9,996)
    OSM_AGGRESSIVE_SELL_THRESH = 10_002   # Slightly tighter (was 10,004)
    OSM_FIXED_L1_OFFSET = 2               # Fixed 2 ticks instead of dynamic 3–5
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # --- Osmium (pass ts via data for spread anticipation) ---
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Fixed L1 offset (forensically proven)
        l1_bid_offset = self.OSM_FIXED_L1_OFFSET + 1   # buy deeper
        l1_ask_offset = max(2, self.OSM_FIXED_L1_OFFSET - 1)  # sell tighter

        # L2/L3 remain dynamic
        base_l2 = max(5, min(9, round(7 * spread_factor)))
        base_l3 = max(9, min(13, round(11 * spread_factor)))

        # Spread anticipation: 1.3x size multiplier every 10k ticks
        ts = data.get("current_ts", 0)
        spread_mult = 1.3 if (ts % 10000 < 2000) else 1.0
        l1_sz = int(self.OSM_L1_SIZE * spread_mult)
        l2_sz = int(self.OSM_L2_SIZE * spread_mult)
        l3_sz = int(self.OSM_L3_SIZE * spread_mult)

        levels = [
            (l1_bid_offset, l1_ask_offset, l1_sz),
            (base_l2, base_l2, l2_sz),
            (base_l3, base_l3, l3_sz),
        ]
