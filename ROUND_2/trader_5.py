# trader.py — IMC Prosperity 4, Round 2 (Final Recalibrated)
# Precision‑tuned to exploit extra flow while preserving core edge.
...
class Trader:
    ...
    # ── Pepper (proven trend‑follow) ──────────────────────────────
    PEPPER_SLOPE   = 0.101
    PEPPER_BUY_TOL = 13           # slightly wider for safety
    ENDGAME_START  = 9_600

    # ── Osmium fair value EMA ─────────────────────────────────────
    OSM_FAIR_INIT  = 10_001
    OSM_EMA_ALPHA  = 0.10         # faster tracking

    # ── Osmium asymmetric levels (sizes increased) ────────────────
    OSM_LEVELS = [
        (1, 1, 35),   # L1: larger size on tightest quote
        (2, 1, 28),
        (3, 1, 22),
        (4, 2, 18),
        (6, 2, 12),
    ...
    OSM_MR_THRESH = 1
    OSM_MR_SIZE   = 25            # larger captures
    ...
    # ──────────────────────────────────────────────────────────────
    def bid(self) -> int:
        """Market Access Fee – increased to confidently clear median."""
        return 6_000
    ...
    # ──────────────────────────────────────────────────────────────
    # OSMIUM: micro‑price + asymmetric levels + larger sizes
    # ──────────────────────────────────────────────────────────────
