# trader.py — IMC Prosperity 4, Round 2 (Final Safe Optimizations for 9k+)
import json
import math
...
class Trader:
    ...
    # --- Pepper (8,678 baseline + scalp reserve) ---
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 4
    ENDGAME_START = 92_000
    SCALP_RESERVE = 6                # ← Enabled (was 0)
    SCALP_DIP = 8
    SCALP_EXIT = 5
    SCALP_SIZE = 10

    # --- Osmium (8,678 baseline + safe upgrades) ---
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA = 0.05             # ← Slightly faster (was 0.02)
    OSM_PASSIVE_BID_OFFSET = 5
    OSM_PASSIVE_ASK_OFFSET = 5
    OSM_PASSIVE_SIZE = 19
    OSM_MR_THRESH = 8
    OSM_MR_MAX_QTY = 24
    OSM_AGGRESSIVE_BUY_THRESH = 9_999    # ← One tick tighter (was 10_000)
    OSM_AGGRESSIVE_SELL_THRESH = 10_004  # ← One tick higher (was 10_003)
    OSM_SELL_COOLDOWN = 300

    # Now enable the safe "smart" features
    OSM_USE_LADDERED_AGGRESSION = False   # Keep off (adds complexity)
    ...
    def _pepper_orders(
        ...
        if not is_endgame:
            ...
            # Scalp (now with headroom reservation)
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Aggressive buying (tightened threshold)
        ...
        # Aggressive selling with cooldown (raised threshold)
        ...
        # Passive quotes with inventory skew (now enabled)
        skew = 0
