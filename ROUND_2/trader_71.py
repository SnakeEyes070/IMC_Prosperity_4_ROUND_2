# trader.py — IMC Prosperity 4, Round 2 (Claude‑Forensic Optimized)
import json
import math
...
class Trader:
    ...
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 4
    ENDGAME_START = 92_000          # Keep – no gain from moving
    SCALP_RESERVE = 0
    SCALP_DIP = 6                   # ← 8 → 6 (+945 XIRECs expected)
    SCALP_EXIT = 4                  # ← 5 → 4 (+60 XIRECs expected)
    SCALP_SIZE = 10

    # --- Osmium (Claude‑optimized) ---
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA = 0.02
    OSM_PASSIVE_BID_OFFSET = 6      # ← 5 → 6 (+80 XIRECs expected)
    OSM_PASSIVE_ASK_OFFSET = 6      # ← 5 → 6 (+80 XIRECs expected)
    OSM_PASSIVE_SIZE = 19
    OSM_MR_THRESH = 8
    OSM_MR_MAX_QTY = 24
    OSM_AGGRESSIVE_BUY_THRESH = 10_000
    OSM_AGGRESSIVE_SELL_THRESH = 10_002   # ← 10_003 → 10_002 (+480 XIRECs)
    OSM_SELL_COOLDOWN = 0                  # ← 300 → 0 (+72 XIRECs)

    # Smart features – now selectively enabled
    OSM_USE_LADDERED_AGGRESSION = False
    OSM_USE_PASSIVE_SKEW = True            # ← Enabled (+150 XIRECs)
    ...
    def bid(self) -> int:
        return 2_250   # Low‑risk lottery ticket
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Aggressive selling – cooldown removed, threshold lowered to 10_002
        ...
        # Passive quotes – offsets 6, inventory skew enabled
        skew = 0
