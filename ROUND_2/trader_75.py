# trader.py — IMC Prosperity 4, Round 2 (Osmium‑Optimized for Net 4k)
import json
import math
...
class Trader:
    ...
    # --- Osmium (Optimized for net 4k) ---
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA = 0.02
    OSM_PASSIVE_BID_OFFSET = 7        # ← 5 → 7 (stay outside market spread)
    OSM_PASSIVE_ASK_OFFSET = 7        # ← 5 → 7
    OSM_PASSIVE_SIZE = 19
    OSM_MR_THRESH = 8
    OSM_MR_MAX_QTY = 24
    OSM_AGGRESSIVE_BUY_THRESH = 10_000
    OSM_AGGRESSIVE_SELL_THRESH = 10_002   # ← 10_003 → 10_002 (capture missed sells)
    OSM_SELL_COOLDOWN = 0             # ← 300 → 0 (no unnecessary blocking)

    # Inventory skew – critical for reducing negative fills
    OSM_USE_PASSIVE_SKEW = True       # ← Enabled
    OSM_SKEW_PER_UNIT = 0.1

    # Imbalance filter – avoid trading into strong pressure
    OSM_IMBALANCE_THRESH = 0.15

    # Keep laddered aggression off (simplicity)
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Micro‑price fair value (volume‑weighted)
        ...
        # Aggressive mean reversion – only when imbalance is favorable
        ...
        # Aggressive taking – strict imbalance gate
        ...
        # Inventory skew – reduce unwanted inventory
        skew = int(round(self.OSM_SKEW_PER_UNIT * pos))

        # Passive quotes – wider offsets, strict imbalance filter
