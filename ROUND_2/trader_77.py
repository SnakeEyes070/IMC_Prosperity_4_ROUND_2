# trader.py — IMC Prosperity 4, Round 2 (Claude‑Forensic Simple Osmium)
# Pepper: 8,678 proven trend‑following.
# Osmium: static fair value, passive quotes only, no aggressive sells.
...
class Trader:
    ...
    # --- Osmium (Claude Forensic Simple) ---
    # Static parameters derived from log analysis
    OSM_FAIR = 10_003          # Mean mid = 10,004; static anchor avoids noise
    OSM_OFFSET = 6             # Bid 9,997, Ask 10,009 – inside wide spread
    OSM_AGG_THRESHOLD = 4      # Buy only when ask ≤ 9,999 (FAIR – 4)
    ...
    def _osmium_orders(self, state: TradingState) -> List[Order]:
        ...
        # Aggressive buy: only when ask is genuinely dislocated
        if best_ask is not None and best_ask <= self.OSM_FAIR - self.OSM_AGG_THRESHOLD and buy_cap > 0:
        ...
        # Passive bid
        if buy_cap > 0:
            orders.append(Order(self.OSMIUM, self.OSM_FAIR - self.OSM_OFFSET, buy_cap))

        # Passive ask (NO aggressive sells)
        if sell_cap > 0:
            orders.append(Order(self.OSMIUM, self.OSM_FAIR + self.OSM_OFFSET, -sell_cap))
