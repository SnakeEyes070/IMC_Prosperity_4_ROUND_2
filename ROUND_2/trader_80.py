# trader.py — IMC Prosperity 4, Round 2 (Seed‑Adaptive Final)
import json
import math
...
class Trader:
    ...
    # Good seed parameters
    OSM_PASSIVE_SIZE_GOOD = 15
    OSM_AGG_BUY_SIZE_GOOD = 20

    # Bad seed parameters (conservative)
    OSM_PASSIVE_SIZE_BAD = 8
    OSM_AGG_BUY_SIZE_BAD = 10

    # Seed detection window
    SEED_DETECTION_WINDOW = 500
    ...
    def bid(self) -> int:
        return 2_100   # Your specified bid
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if prev_ts > self.NEW_DAY_THRESH and ts < self.NEW_DAY_THRESH:
            ...
            data.pop("osm_seed_type", None)   # Reset seed detection each day
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        # Seed detection: check if we got a fill in the first 500 ticks
        if "osm_seed_type" not in data:
            data["osm_seed_type"] = "unknown"
            data["osm_seed_check_ts"] = ts

        if data["osm_seed_type"] == "unknown" and ts <= self.SEED_DETECTION_WINDOW:
            # Check own_trades for an Osmium sell fill (we hit the ask)
            for trade in state.own_trades.get(self.OSMIUM, []):
                if trade.seller == "SUBMISSION" and trade.price >= 10010:
                    data["osm_seed_type"] = "good"
                    break
            if ts == self.SEED_DETECTION_WINDOW and data["osm_seed_type"] == "unknown":
                data["osm_seed_type"] = "bad"

        # Set parameters based on detected seed
        if data.get("osm_seed_type") == "good":
            passive_size = self.OSM_PASSIVE_SIZE_GOOD
            agg_buy_size = self.OSM_AGG_BUY_SIZE_GOOD
        else:
            passive_size = self.OSM_PASSIVE_SIZE_BAD
            agg_buy_size = self.OSM_AGG_BUY_SIZE_BAD
        ...
        if best_ask is not None and best_ask <= self.OSM_FAIR - self.OSM_AGG_BUY_THRESHOLD:
            if buy_cap > 0 and imbalance > -self.OSM_IMBALANCE_THRESH:
                vol = min(agg_buy_size, buy_cap, -od.sell_orders.get(best_ask, 0))
                if vol > 0:
        ...
        # Passive ask – NO aggressive sells
