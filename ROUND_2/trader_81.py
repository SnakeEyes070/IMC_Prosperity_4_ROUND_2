# trader.py — IMC Prosperity 4, Round 2 (Forensic Anchor + Osmium Fixes)
import json
import math
...
class Trader:
    ...
    # --- Pepper (8,678 baseline + anchor fallback fix) ---
    PEPPER_SLOPE = 0.001
    PEPPER_BUY_TOL = 4
    ...
    # Anchor fallback spread (median observed = 14 ticks)
    PEPPER_ANCHOR_FALLBACK_SPREAD = 14

    # --- Osmium (8,678 baseline + two proven fixes) ---
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA = 0.02
    OSM_PASSIVE_BID_OFFSET = 3          # ← 5 → 3 (captures 5× more fills)
    OSM_PASSIVE_ASK_OFFSET = 5          # ← unchanged (prevents accelerating short)
    OSM_PASSIVE_SIZE = 19
    OSM_MR_THRESH = 8
    OSM_MR_MAX_QTY = 24
    OSM_AGGRESSIVE_BUY_THRESH = 10_001  # ← 10_000 → 10_001 (captures 6 more ask-dips)
    ...
    # ---------- Pepper (with anchor fallback fix) ----------
    def _pepper_anchor(self, state: TradingState) -> float:
        ...
        if od.buy_orders:
            # FIX: When no asks exist, estimate ask = max_bid + median_spread
            return float(max(od.buy_orders.keys()) + self.PEPPER_ANCHOR_FALLBACK_SPREAD)
        return 12_000.0
    ...
    # ---------- Osmium (with bid offset and aggressive buy threshold fixes) ----------
